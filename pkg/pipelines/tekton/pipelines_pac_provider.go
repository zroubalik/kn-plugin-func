...
func (pp *PipelinesProvider) ConfigurePAC(ctx context.Context, f fn.Function, metadata pipelines.PacMetadata) error {

	if err := validatePipeline(f); err != nil {
		return err
	}

	if metadata.ConfigureLocalResources {
		if err := pp.createLocalResources(ctx, f); err != nil {
			return err
		}
	}

	if metadata.ConfigureClusterResources || metadata.ConfigureRemoteResources {
		if metadata.WebhookSecret == "" {
			metadata.WebhookSecret = random.AlphaString(10)
			...
			if err != nil {
				...
				if ok {
					metadata.WebhookSecret = val
				}
			}
		}
	}

	if metadata.ConfigureClusterResources {
		if err := pp.createClusterResources(ctx, f, metadata); err != nil {
			return err
		}
	}

	if metadata.ConfigureRemoteResources {
		if err := pp.createRemoteResources(ctx, f, metadata); err != nil {
			return err
		}
	}
...
func (pp *PipelinesProvider) createRemoteResources(ctx context.Context, f fn.Function, metadata pipelines.PacMetadata) error {
	repoOwner, repoName, err := git.GetRepoOwnerAndNameFromUrl(f.Build.Git.URL)
	if err != nil {
		return err
	}
